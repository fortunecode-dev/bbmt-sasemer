  ...
  try {
    ...
    try {
      if (username != "RemesilloBot") {
        ...
        if (pinnedMsgId) {
          try { await tg.pinChatMessage(chat_id, pinnedMsgId, { disable_notification: true }); } catch (e) { tg.sendMessage(chat_id, JSON.stringify(e)) }
        }
      }
    }
    catch (e) { tg.sendMessage(chat_id, JSON.stringify(e)) }
